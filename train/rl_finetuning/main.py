...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=1000,
        help=
        'Number of steps we do during training before we save a model checkpoint'
    )
    parser.add_argument(
        '--data_output_path',
        type=str,
...
def main():
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            ## saving model
            if step % args.steps_per_checkpoint == 0 and step != 0:
                save_model(rlhf_engine, args, tokenizer, step)
...
def save_model(rlhf_engine, args, tokenizer, step):
    if args.output_dir is not None:
        print_rank_0(f'saving model ... step: {step}')
